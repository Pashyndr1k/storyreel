...
// (dialogue/sound density). Rendered on every stage after Stage 2; Stage 6
// passes `playhead` (seconds) to sync a cursor with the timeline.
const W = 720;
const H = 96;
const PAD_X = 34;
const PAD_Y = 10;

export default function DynamicsVisualizer({ plan, playhead = null, defaultOpen = false }) {
  const { t } = useI18n();
  ...
  return (
    <div className={`dyn-viz ${open ? 'open' : ''}`}>
      <button type="button" className="dyn-viz-head" onClick={() => setOpen((v) => !v)}>
        <span className="dyn-viz-title">{t('dyn.title')}</span>
        <span className="dyn-viz-meta">
          {t(`dyn.curve_${plan.global_pacing_curve}`)} · {blocks.length} {t('dyn.blocks')}
        </span>
        <span className="dyn-viz-chevron">{open ? '▾' : '▸'}</span>
      </button>
      {open && (
